...
class NoteViewHolder(private val noteView: View, noteListener: NoteAdapter.Listener?) : ViewHolder(noteView) {
    ...
    fun bind(note: Note) {
        ...
        id = note.id
        Glide.with(noteView).load(Uri.parse(note.imageUri)).centerCrop().into(imageView)
